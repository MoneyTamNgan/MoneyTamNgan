...
const DOCUMENT_LIST_TIMEOUT = 60000;
...
function isTimeoutError(error) {
    return error?.name === 'TimeoutError'
        || /timed?\s*out|timeout|operation was aborted/i.test(String(error?.message || error));
}

export function unresolvedDocumentLookupError(zipResult, attachments) {
    return attachments.length === 0 ? zipResult?.unavailableError || null : null;
}
...
async function fetchJsonInBrowserSession(page, url, timeout = DOCUMENT_LIST_TIMEOUT) {
    const result = await page.evaluate(async ({ requestUrl, timeoutMs }) => {
    ...
    }, { requestUrl: url, timeoutMs: timeout });
    if (result.error) throw new Error(`e-GP API request failed: ${result.error}`);
    if (result.status < 200 || result.status >= 300) {
        throw new Error(`e-GP API returned HTTP ${result.status}`);
    }
    try {
        return JSON.parse(result.text);
    } catch {
        throw new Error('e-GP API returned invalid JSON');
    }
}

async function resolveLatestEbindingAttachment(page, rowIndex, projectId) {
    const requestPromise = page.waitForRequest(
        request => request.url().includes('/getTorZipList?'),
        { timeout: NAV_TIMEOUT }
    );
    ...
    // Capture the URL generated by the official application, then issue a
    // separate fetch in the same browser session. This avoids waiting for the
    // modal's Angular subscription, which is frequently terminated mid-flight.
    const listRequest = await requestPromise;
    const listPayload = await fetchJsonInBrowserSession(page, listRequest.url());
    const archives = Array.isArray(listPayload?.data) ? listPayload.data : [];
    if (!archives.length) throw new Error('e-GP returned an empty e-Bidding ZIP list');

    const latestIndex = archives.length - 1;
    const latest = archives[latestIndex];
    const approvalPath = latestIndex === 0
        ? `/egp-approval-service/apv-common/infoProcureDocAnnounZipTemp?projectId=${encodeURIComponent(projectId)}`
        : `/egp-approval-service/apv-common/infoProcureDocAnnounZipAdj?itemNo=${latestIndex}`
            + `&projectId=${encodeURIComponent(projectId)}`;
    const approvalUrl = new URL(approvalPath, listRequest.url()).href;
    const approvalPayload = await fetchJsonInBrowserSession(page, approvalUrl);
    const approvals = Array.isArray(approvalPayload?.data)
        ? approvalPayload.data
        : [approvalPayload?.data].filter(Boolean);
    const approved = approvals.at(-1);
    if (!approved?.zipId) throw new Error('e-GP returned no ZIP file identifier');

    return {
        name: approved.buildName1 || latest.buildName || 'e-bidding.zip',
        url: new URL(
            `/egp-upload-service/v1/downloadFileTest?fileId=${encodeURIComponent(approved.zipId)}`,
            listRequest.url()
        ).href,
        type: 'zip',
    };
...
async function downloadEbindingZip(page, projectId, storageDir) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
            // The Angular detail page often renders project metadata several
            // seconds before its related-document table. Wait for the target
            // row instead of repeatedly reloading a healthy in-flight page.
            await page.waitForFunction(
                () => Array.from(document.querySelectorAll('tbody tr'))
                    .some(row => /ร่างเอกสารประกวดราคา\s*\(e-?bidding\)/i.test(row.innerText || '')),
                { timeout: DOCUMENT_LIST_TIMEOUT }
            );
            const rowIndex = await findEbindingRowIndex(page);
            if (rowIndex >= 0) {
                const attachment = await resolveLatestEbindingAttachment(
                    page,
                    rowIndex,
                    projectId
                );
            ...
            lastError = error;
            console.warn(`   ⚠️  e-GP document-list attempt ${attempt}/${attempts} failed: ${error.message}`);
        }
        ...
        if (attempt < attempts) {
            ...
            await sleep(2000);
            try {
                // e-GP keeps background requests open, so networkidle2 can time
                // out even after the document table has rendered successfully.
                await page.reload({ waitUntil: 'domcontentloaded', timeout: 70000 });
                await ensureRenderedPage(page);
            } catch (error) {
                lastError = error;
                if (!isTimeoutError(error)) throw error;
                console.warn(`   ⚠️  e-GP reload timed out; continuing retry ${attempt + 1}/${attempts}`);
            }
        }
    }
    if (lastError) {
        console.warn(`   ⚠️  e-GP ZIP list remained unavailable; trying visible document links: ${lastError.message}`);
        return { unavailableError: lastError };
    }
    return null;
}
...
function configuredMaxDownloadBytes() {
    const configuredMb = Number(process.env.TOR_MAX_FILE_SIZE_MB || 250);
...
function configuredDownloadTimeoutMs() {
    const value = Number(process.env.EGP_DOWNLOAD_TIMEOUT_MS || 300000);
    return Number.isInteger(value) && value >= 30000 && value <= 600000
        ? value
        : 300000;
}
...
export async function downloadAttachment(attachment, options) {
    ...
    const response = await fetchDocument(
        requestedUrl.href,
        headers,
        AbortSignal.timeout(configuredDownloadTimeoutMs())
    );
...
export async function scrapeProjectTOR(projectId, existingBrowser = null, options = {}) {
    ...
    try {
        ...
        if (attachments.length > 0) {
            ...
            const lookupError = unresolvedDocumentLookupError(ebiddingZip, attachments);
