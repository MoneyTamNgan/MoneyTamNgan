 ...
 *
 * Uses Puppeteer to navigate the Thai e-GP procurement website,
 * search for projects by project_id, and
 * extract TOR PDF/ZIP attachment download links.
 */
 ...
 *
 * Strategy:
 * 1. Open the current e-GP announcement SPA
 * 2. Search by project ID (and infer the fiscal year for older projects)
 * 3. Open the matching project detail page
 * 4. Extract direct PDF/ZIP/document URLs from the rendered detail page
 *
...
export async function scrapeProjectTOR(projectId, existingBrowser = null, options = {}) {
    ...
    try {
        ...
        console.log(`   🔍 Scraping project ${normalizedProjectId}...`);
        ...
        if (attachments.length > 0) {
            ...
            result.pdf_url = primaryAttachment.url;
            result.pdf_path = downloadedFile.path;
            result.pdf_size = downloadedFile.size;
            result.pdf_content_type = downloadedFile.contentType;
            primaryAttachment.local_path = downloadedFile.path;
