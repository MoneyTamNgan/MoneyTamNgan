...
function documentUpdate(result, sourceType, retainFiles) {
    return {
        ...
        'document.extracted_files': (result.extracted_pdfs || []).map(file => ({
            entryName: file.entryName,
            filename: file.filename,
            size: file.size,
            ...(retainFiles ? { path: file.path } : {}),
        })),
        ...(retainFiles ? {
...
export async function processProject(projectId, options = {}, {
    ...
    linkExistingSummary = linkExistingSummaryBundle,
} = {}) {
    ...
    const acquisitionUpdate = {
        $set: compactSet(documentUpdate(result, acquisition.sourceType, retainFiles)),
    };
    if (!retainFiles) acquisitionUpdate.$unset = {
        pdf_path: '',
        'document.local_path': '',
        'document.gcs_uri': '',
        'document.archive_path': '',
    };
    await Project.updateOne({ _id: project._id }, acquisitionUpdate);
    ...
    const storageSet = compactSet({
        'document.record_id': primaryDocument._id,
        ...
        'document.status': retainFiles ? 'stored' : 'linked',
    ...
    try {
        ...
        const textUpdate = { $set: {
            'document.text_storage': textArtifact?.gcsUri ? 'gcs'
                : retainFiles ? 'local' : 'mongodb',
            ...
            'ocr.review_pages': textResult.reviewPages || [],
            'processing.status': 'text_extracted',
        } };
        if (textArtifactUri) textUpdate.$set['document.text_uri'] = textArtifactUri;
        else textUpdate.$unset = { 'document.text_uri': '' };
        if (dualWrite) {
    ...
    const alreadyProcessed = ['completed', 'review_required'].includes(project.processing?.status)
        ...
        && project.ocr?.processor_version === textResult.fingerprint
        && project.processing?.model === (process.env.VERTEX_MODEL || 'gemini-2.5-flash')
        && project.processing?.prompt_version === TOR_PROMPT_VERSION;
    if (alreadyProcessed) {
        const reusedStatus = textResult.needsReview ? 'review_required' : project.processing.status;
        if (dualWrite) {
            const linkedSummary = await linkExistingSummary(
                project,
                primaryDocument,
                extractionRun,
                textResult,
            ...
            if (!linkedSummary) {
                // A legacy project can claim completion while its summary record is missing.
                // Continue to Vertex below so normalized storage is made complete.
            } else {
                return {
                    projectId: project.project_id,
                    status: reusedStatus,
                    reused: true,
                    documentSummaryId: String(linkedSummary._id),
                };
            }
        } else {
            await Project.updateOne({ _id: project._id }, { $set: { 'processing.status': reusedStatus } });
            return { projectId: project.project_id, status: reusedStatus, reused: true };
        }
    }
