...
export default async function TorDocumentPage({ params }) {
  ...
  return <AppShell title="สรุปเอกสาร TOR">
    ...
    <section className="two-column"><article className="content-card"><p className="eyebrow">คุณสมบัติผู้ยื่นข้อเสนอ</p>{project.extracted_data.qualifications.length ? <ul>{project.extracted_data.qualifications.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul> : <p>ยังไม่มีข้อมูลคุณสมบัติ</p>}</article><article className="content-card"><p className="eyebrow">ขอบเขตงาน</p>{project.extracted_data.scope_of_work.length ? <ul>{project.extracted_data.scope_of_work.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul> : <p>ยังไม่มีข้อมูลขอบเขตงาน</p>}</article></section>
    <section className="content-card"><p className="eyebrow">เทคโนโลยีที่สกัดได้</p>{project.extracted_data.tech_stack.length ? <div className="tag-list">{project.extracted_data.tech_stack.map((item, index) => <span className="tag software" key={`${item}-${index}`}>{item}</span>)}</div> : <p>ยังไม่มีข้อมูลเทคโนโลยี</p>}</section>
  </AppShell>;
}
